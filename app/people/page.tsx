...
export default function PeoplePage() {
  ...
  const fetchPeople = useCallback(async (query: string) => {
    ...
    try {
      const url = query
        ? `/api/gramps/people?search=${encodeURIComponent(query)}`
        : '/api/gramps/people'
    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-[13px] text-gray-600">
          {people.length > 0
            ? `${people.length.toLocaleString()} results`
            : hasSearched
            ? 'No results found'
        ...
        {people.length > 0 && (
          <div className="mt-3 pt-3 border-t border-[#D3D1C7]">
            <p className="text-[11px] text-gray-600">
              Showing <span className="font-semibold">{people.length.toLocaleString()}</span> people
            </p>
          </div>
        )}
      </div>
      ...
      <div className="bg-[#FDFCFA] border border-[#D3D1C7] rounded-lg overflow-hidden">
        {isLoading ? (
          <div className="p-8 text-center text-gray-500 text-[13px]">Searching Gramps...</div>
        ) : !hasSearched ? (
          <div className="p-8 text-center text-gray-400 text-[13px]">
            Start typing to search the family tree
          </div>
